...
public class OpenIDManager {
	...
	public OpenIDManager() throws OpenIDException {
		if(setting==null){
			setting=new OpenIDSettingLoader().getSetting("openidSetting.properties");
			respone=new OpenIDSettingLoader().getSetting("openidCheck.properties");
		}
	}
	...
	public boolean checkAuthentication(Map request){
		...
		return checkAuthenticationWithStringMap(re);
	}
	...
	private String getIncomingParameters(String param,Map<String,String> request) throws UnsupportedEncodingException{
		for(String key: respone.getProp().stringPropertyNames()){
			String realKey=respone.getProp().getProperty(key);
				param+= "&" + realKey+"=" 
						+ URLEncoder.encode(request.get(realKey.toString()), charset);
	...
	private String getSettingParameters() throws UnsupportedEncodingException{
		String url  = String.format(		
				...
				"openid.realm=%s"									
				,										
				URLEncoder.encode(setting.getProp().getProperty("openid.return_to"), charset),
				URLEncoder.encode(setting.getProp().getProperty("openid.realm"), charset)
				);
		return url;
	}
}
